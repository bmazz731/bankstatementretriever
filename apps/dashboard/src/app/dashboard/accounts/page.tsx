...
const PlaidLinkButton = dynamic(
  () => import("@/components/plaid/plaid-link-button"),
  {
    ssr: false,
    loading: () => (
...
export default function AccountsPage() {
  ...
  const filteredAccounts = accountData.filter((account) => {
    ...
    const institutionName = safeAccess(
      account,
    ...
    const matchesStatus =
      statusFilter === "all" ||
      safeAccess(account, ["status"], "") === statusFilter;
            ...
            {isLoading ? (
              ...
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  {!searchTerm && statusFilter === "all" && <PlaidLinkButton />}
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-4">
                {filteredAccounts.map((account) => {
                  ...
                  const connectionStatus =
                    account.connection?.status || "unknown";
                  const lastSync =
                    account.last_statement_check ||
                    account.connection?.last_sync;
