...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://api.bankstatementretriever.com";
...
class ApiClient {
  ...
  async getStatements(
    ...
    const query = searchParams.toString();
    return this.request(
      `/api/statements/${accountId}${query ? `?${query}` : ""}`,
    );
  }
  ...
  async testWebhookDestination(destinationId: string) {
    return this.request(`/api/destinations/${destinationId}/test`, {
      method: "POST",
    });
  }
