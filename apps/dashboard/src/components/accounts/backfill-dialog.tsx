...
export function BackfillDialog({ account, open, onOpenChange }: BackfillDialogProps) {
  ...
  const backfillMutation = useMutation({
    mutationFn: ({ accountId, range_start, range_end }: {
      accountId: string
      range_start: string
      range_end: string
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Retrieve historical statements for {account.name}.
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {rangeStart && rangeEnd && (
            <div className="p-3 bg-muted rounded-md">
              <p className="text-sm">
                <strong>Preview:</strong> Statements from {formatDate(rangeStart)} to {formatDate(rangeEnd)}
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                This may take several minutes to complete depending on the date range.
              </p>
            </div>
          )}
