...
export const apiLogger = createMiddleware<{ Bindings: Env }>(async (c, next) => {
...
export const rateLimit = (
  ...
  return createMiddleware<{ Bindings: Env }>(async (c, next) => {
    try {
      ...
      const key = keyGenerator ? keyGenerator(c) : 
...
export const extractUserContext = createMiddleware<{ Bindings: Env }>(async (c, next) => {
  try {
...
export const idempotency = createMiddleware<{ Bindings: Env }>(async (c, next) => {
  ...
  if (idempotencyKey && ['POST', 'PUT', 'PATCH'].includes(c.req.method)) {
    ...
    const existingResponse = await c.env.BSR_CACHE.get(`idempotency:${idempotencyKey}`)
