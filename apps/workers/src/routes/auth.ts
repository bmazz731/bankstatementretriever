  ...
  try {
    ...
    const response = await fetch(`${supabaseUrl}/auth/v1/resend`, {
      ...
      body: JSON.stringify({
        ...
        options: {
          emailRedirectTo:
            "https://app.bankstatementretriever.com/auth/callback",
        },
      }),
    });
    ...
    console.log(
      `Resend confirmation for ${email}, Response: ${response.status}, Body: ${result}`,
    );
...
auth.post("/resend-simple", async (c) => {
  try {
    ...
    return c.json({
      success: true,
      message:
        "If an account with this email exists, a confirmation email has been sent.",
      email: email,
