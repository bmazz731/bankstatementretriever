...
api.get("/accounts", async (c) => {
  try {
    ...
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/accounts?${queryParams}`,
      {
        headers: {
...
api.delete("/accounts/:id", async (c) => {
  ...
  try {
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/accounts?id=eq.${accountId}`,
      {
        method: "PATCH",
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
...
api.get("/statements/:accountId", async (c) => {
  ...
  try {
    ...
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/statements?${queryParams}`,
      {
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
...
api.post("/accounts/:id/backfill", async (c) => {
  ...
  try {
    ...
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/accounts?id=eq.${accountId}&select=*,connection(user_id)`,
      {
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
...
api.post("/destinations", async (c) => {
  try {
    ...
    if (type === "webhook") {
      const webhookValidation = validateInput(
        config,
        ValidationSchemas.webhookConfig,
      );
...
api.get("/destinations", async (c) => {
  try {
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/destinations?active=eq.true&order=created_at.desc`,
      {
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
...
api.post("/destinations/:id/test", async (c) => {
  ...
  try {
    // Get destination
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/destinations?id=eq.${destinationId}&kind=eq.webhook&select=*`,
      {
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
...
api.post("/routes", async (c) => {
  try {
    const { account_id, destination_id, folder_override, filename_template } =
      await c.req.json();
    ...
    const [accountCheck, destinationCheck] = await Promise.all([
      fetch(
        `${c.env.SUPABASE_URL}/rest/v1/accounts?id=eq.${account_id}&select=id`,
        {
          headers: {
            apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
            Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
      ...
      fetch(
        `${c.env.SUPABASE_URL}/rest/v1/destinations?id=eq.${destination_id}&select=id`,
        {
          headers: {
            apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
            Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
    ...
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/account_destinations`,
      {
        method: "POST",
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
        ...
        body: JSON.stringify({
          ...
          filename_template:
            filename_template ||
            "{institution}_{account_name}_{statement_date}.pdf",
          is_active: true,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        }),
...
api.put("/notifications/preferences/:accountId", async (c) => {
  ...
  try {
        ...
        return c.json(
          {
            error: "BSR_VALIDATION_ERROR",
            message:
              "Each preference must have channel, event_type, and enabled fields",
          },
          400,
        );
      }
    ...
    const accountResponse = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/accounts?id=eq.${accountId}&select=id`,
      {
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${c.env.SUPABASE_SERVICE_ROLE_KEY}`,
    ...
    await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/notification_preferences?account_id=eq.${accountId}`,
      {
        method: "DELETE",
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
    ...
    const response = await fetch(
      `${c.env.SUPABASE_URL}/rest/v1/notification_preferences`,
      {
        method: "POST",
        headers: {
          apikey: c.env.SUPABASE_SERVICE_ROLE_KEY,
