...
plaid.post("/debug_dns", async (c) => {
  try {
    ...
    const g = (name) =>
      fetch(`https://dns.google/resolve?name=${name}&type=A`).then((r) =>
        r.json(),
      );

    const names = [
      "google.com",
      "production.api.plaid.com",
      "development.api.plaid.com",
      "sandbox.api.plaid.com",
    ];
...
plaid.post("/test_link_token", async (c) => {
  try {
        ...
        return c.json(
          {
            ...
            message:
              "Plaid service configuration incomplete. Missing environment variables.",
    ...
    const linkTokenResponse =
      await plaidService.createLinkToken("test_user_123");
  ...
  try {
    ...
    if (error instanceof PlaidAPIError) {
      ...
      if (error.plaidError.error_code === "INVALID_PRODUCT") {
        userMessage =
          "The statements product is not available for your Plaid environment. Please contact support.";
      } else if (error.plaidError.error_code === "INVALID_CREDENTIALS") {
        userMessage =
          "Plaid API credentials are invalid. Please check configuration.";
      } else if (error.plaidError.error_code === "PRODUCT_NOT_ENABLED") {
        userMessage =
          "The required Plaid product is not enabled for your account.";
      }
    ...
    return c.json(
      {
        error: "BSR_INTERNAL_ERROR",
        message:
          "Failed to create link token. Please try again or contact support.",
      },
      500,
    );
  }
...
plaid.post("/exchange_public_token", async (c) => {
  try {
    ...
    try {
      ...
      console.error(
        "PlaidService/TokenEncryption initialization error:",
        error,
      );
    ...
    if (!existingOrg) {
      console.log(
        "Organization does not exist, creating it with id:",
        user.org_id,
      );
      ...
      if (!existingUser) {
        console.log(
          "Creating user and organization using transaction with deferred constraints",
        );
        ...
        return c.json(
          {
            error: "BSR_DATABASE_ERROR",
            message:
              "Failed to store connection: Organization not found. Please contact support.",
            debug: {
    ...
    for (const account of accountsResponse.accounts) {
      ...
      try {
        ...
        console.log(
          `Account ${account.account_id} does not support statements:`,
          error,
        );
      }
    ...
    if (validatedInput.backfill_months > 0) {
      ...
      console.log(
        `Backfill requested for ${validatedInput.backfill_months} months`,
      );
    }
