...
async function updateConnectionStatus(itemId: string, status: string, env: Env) {
  try {
    const response = await fetch(`${env.SUPABASE_URL}/rest/v1/connections?plaid_item_id=eq.${itemId}`, {
      method: 'PATCH',
      headers: {
...
async function updateOrgStatusByStripeCustomer(customerId: string, status: string, env: Env) {
  // Note: This assumes we store stripe_customer_id in organizations table
  // Implementation depends on billing integration
  console.log(`TODO: Update org status for Stripe customer ${customerId} to ${status}`)
}

// Update organization plan by Stripe customer ID
async function updateOrgPlanByStripeCustomer(customerId: string, plan: string, env: Env) {
  console.log(`TODO: Update org plan for Stripe customer ${customerId} to ${plan}`)
}
