...
export class PlaidService {
  ...
  constructor(env: Env) {
    ...
    if (!env.DOMAIN) {
      throw new Error(
        "DOMAIN environment variable is required for webhook URL",
      );
    }
    ...
    if (!this.baseUrl) {
      throw new Error(
        `Invalid PLAID_ENV: ${env.PLAID_ENV}. Must be 'sandbox', 'development', or 'production'`,
      );
    }
  }
  ...
  async createLinkToken(userId: string): Promise<LinkTokenCreateResponse> {
    ...
    try {
      return await this.makeRequest<LinkTokenCreateResponse>(
        "/link/token/create",
        request,
      );
    } catch (error) {
      ...
      if (
        ...
        console.log(
          "Statements product not available, retrying with transactions",
        );
        ...
        return this.makeRequest<LinkTokenCreateResponse>(
          "/link/token/create",
          fallbackRequest,
        );
      }
  ...
  async exchangePublicToken(
    ...
    return this.makeRequest<ItemPublicTokenExchangeResponse>(
  ...
  async getStatements(
    ...
    return this.makeRequest<StatementsListResponse>(
      "/statements/list",
      request,
    );
  }
  ...
  private async makeRequest<T>(endpoint: string, data: any): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `Plaid API ${endpoint} failed with status ${response.status}:`,
          responseData,
        );
