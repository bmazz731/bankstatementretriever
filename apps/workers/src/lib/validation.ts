...
function validateField(value: any, schema: FieldSchema, fieldName: string): ValidationError[] {
  const errors: ValidationError[] = []
      ...
      if (schema.properties) {
        ...
        for (const [prop, propSchema] of Object.entries(schema.properties)) {
          if (prop in value && value[prop] !== null && value[prop] !== undefined) {
            const propErrors = validateField(value[prop], propSchema, `${fieldName}.${prop}`)
