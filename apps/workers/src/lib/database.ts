...
export class DatabaseClient {
  ...
  async getStatementsByAccount(accountId: string, options: {
    from?: string
    to?: string
    fileType?: string
    limit?: number
  ...
  async updateDeliveryStatus(deliveryId: string, updates: {
    status: string
    path?: string
    delivered_at?: string
    last_error?: string
  ...
  async getNotificationPreferences(accountId: string) {
    ...
    if (error) throw new Error(`Failed to get notification preferences: ${error.message}`)
    return data
  }
...
export async function executeQuery(env: Env, query: string, params: any[] = []) {
