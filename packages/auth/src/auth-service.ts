...
export class AuthService {
  ...
  async signUp(
    ...
    try {
      ...
      const { token, refreshToken, expiresAt } =
        await this.jwtManager.createToken(user, session.sessionId);
  ...
  async signIn(
    ...
    try {
      ...
      // Verify password
      const passwordValid = await this.passwordValidator.verify(
        password,
        user.password_hash,
      );
      if (!passwordValid) {
      ...
      // Create JWT token
      const { token, refreshToken } = await this.jwtManager.createToken(
        user,
        session.sessionId,
      );
  ...
  async sendMagicLink(
    ...
    try {
      ...
      const token = await this.jwtManager.createMagicLinkToken(
        options.email,
        options.action,
      );
  ...
  async verifyMagicLink(
    ...
    try {
      ...
      // Create JWT token
      const { token: jwtToken, refreshToken } =
        await this.jwtManager.createToken(user, session.sessionId);
  ...
  async changePassword(
    ...
    try {
      ...
      const currentPasswordValid = await this.passwordValidator.verify(
        currentPassword,
        user.password_hash,
      );
