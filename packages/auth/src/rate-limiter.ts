...
export class AuthRateLimiter {
  ...
  async checkLimit(identifier: string): Promise<{
    ...
    if (rateLimit.lockedUntil && now < rateLimit.lockedUntil) {
      const lockoutMinutes = Math.ceil((rateLimit.lockedUntil - now) / (60 * 1000))
      return {
        allowed: false,
        error: {
          code: 'ACCOUNT_LOCKED',
    ...
    if (rateLimit.attempts >= this.config.maxLoginAttempts) {
      ...
      rateLimit.lockedUntil = now + (this.config.lockoutDurationMinutes * 60 * 1000)
  ...
  cleanup(): void {
    ...
    for (const [key, rateLimit] of this.attempts.entries()) {
      ...
      const lockoutExpired = !rateLimit.lockedUntil || now > rateLimit.lockedUntil
