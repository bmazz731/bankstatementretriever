...
export class StatementLearningService {
  ...
  async isInLearningPeriod(
    ...
    try {
      ...
      const daysSinceFirst = Math.floor(
        (Date.now() - firstRecord.available_date.getTime()) /
          (1000 * 60 * 60 * 24),
      );
  ...
  private async getPattern(
    ...
    try {
      ...
      const variance =
        days.reduce((sum, day) => sum + Math.pow(day - avgDay, 2), 0) /
        days.length;
  ...
  async getStatistics(institutionId?: string): Promise<{
    ...
    try {
      ...
      for (const account of accounts) {
        ...
        const inLearning = await this.isInLearningPeriod(
          account.institution_id,
          account.account_id,
        );
      ...
      return {
        ...
        avg_confidence:
          accounts.length > 0 ? totalConfidence / accounts.length : 0,
      };
    } catch (error) {
