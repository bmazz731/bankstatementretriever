...
export class PlaidService {
   ...
   */
  async createLinkToken(userId: string, clientName: string = 'BankStatementRetriever'): Promise<PlaidResult<LinkTokenResponse>> {
    try {
      ...
      const response = await this.makeRequest<LinkTokenResponse>('/link/token/create', request)
      return response
   ...
   */
  async exchangePublicToken(publicToken: string): Promise<PlaidResult<ItemPublicTokenExchangeResponse>> {
    try {
      const request: ItemPublicTokenExchangeRequest = {
        public_token: publicToken
      }

      const response = await this.makeRequest<ItemPublicTokenExchangeResponse>('/link/token/exchange', request)
      return response
   ...
   */
  async getAccounts(accessToken: string): Promise<PlaidResult<AccountsGetResponse>> {
    try {
      const request = {
        access_token: accessToken
      }

      const response = await this.makeRequest<AccountsGetResponse>('/accounts/get', request)
      return response
   ...
   */
  async getInstitution(institutionId: string): Promise<PlaidResult<InstitutionsGetByIdResponse>> {
    try {
      const request = {
      ...
      const response = await this.makeRequest<InstitutionsGetByIdResponse>('/institutions/get_by_id', request)
      return response
  ...
  async getStatements(accessToken: string, accountId: string, options: {
    ...
    try {
      ...
      const response = await this.makeRequest<StatementsListResponse>('/statements/list', request)
      return response
   ...
   */
  async downloadStatement(accessToken: string, statementId: string): Promise<PlaidResult<ReadableStream<Uint8Array>>> {
    try {
   ...
   */
  async storeAccessToken(userId: string, itemId: string, accessToken: string): Promise<boolean> {
    try {
   ...
   */
  async updateConnectionInstitution(itemId: string, institutionId: string, institutionName: string): Promise<void> {
    try {
  ...
  async syncAccounts(itemId: string): Promise<boolean> {
    try {
      ...
      if (item.institution_id) {
        const institutionResult = await this.getInstitution(item.institution_id)
  ...
  private async makeRequest<T>(endpoint: string, data: any): Promise<PlaidResult<T>> {
    try {
