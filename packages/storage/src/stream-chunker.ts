...
export class EdgeStreamChunker implements StreamChunker {
    ...
    try {
      while (bytesRead < totalSize) {
        ...
        const chunkData = buffer.slice(
          0,
          Math.min(this.chunkSize, buffer.length),
        );
  ...
  async createChunkFromRange(
    ...
    if (!rangeStream) {
      throw new Error(
        `Failed to get range ${startByte}-${endByte} from R2 object`,
      );
    }
  ...
  createRateLimitedTransform(
    ...
    return new TransformStream({
      async transform(chunk, controller) {
        ...
        if (
          maxBytesPerSecond &&
          bytesThisSecond + chunk.length > maxBytesPerSecond
        ) {
  ...
  static getRequiredChunks(
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      const isCompleted = completedRanges.some(
        (range) => range.start <= chunkStart && range.end >= chunkEnd,
      );
