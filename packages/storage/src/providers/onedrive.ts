...
export class OneDriveProvider implements StorageProvider {
  ...
  async validateTokens(tokens: StorageTokens): Promise<boolean> {
    return this.oauthService.validateTokens(`${this.baseUrl}/me`, tokens);
  }
        ...
        return {
          ...
          data: {
            bytesUploaded: sessionData.nextExpectedRanges?.[0]
              ? parseInt(sessionData.nextExpectedRanges[0].split("-")[0])
              : 0,
      ...
      if (!response.ok) {
        ...
        return {
          ...
          error: {
            ...
            message:
              errorData.error?.message || "Failed to create sharing link",
          },
        };
      }
