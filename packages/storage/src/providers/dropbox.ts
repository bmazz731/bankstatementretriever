...
export class DropboxProvider implements StorageProvider {
      ...
      if (!response.ok) {
        ...
        return {
          ...
          error: {
            ...
            message: errorData.error_summary || 'Failed to create upload session',
            details: errorData
          }
        }
      }
  ...
  async finalizeUpload(sessionId: string): Promise<StorageResult<StorageFile>> {
    return {
      ...
      error: {
        code: 'NOT_NEEDED',
        message: 'Dropbox uploads are finalized automatically with the last chunk'
      }
    }
  }
  ...
  private async appendToSession(
    ...
    if (!response.ok) {
      ...
      return {
        ...
        error: {
          code: 'CHUNK_UPLOAD_ERROR',
          message: errorData.error_summary || `Upload failed with status ${response.status}`,
    ...
    if (!response.ok) {
      ...
      return {
        ...
        error: {
          ...
          message: errorData.error_summary || `Upload finish failed with status ${response.status}`,
          retryable: response.status >= 500 || response.status === 429
        }
      }
    }
  ...
  async getTemporaryLink(
    ...
    try {
      ...
      return {
        ...
        error: {
          code: 'NETWORK_ERROR',
          message: error instanceof Error ? error.message : 'Failed to get temporary link'
        }
      }
    }
  }
