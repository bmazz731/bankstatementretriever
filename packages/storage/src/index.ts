...
export { EdgeStreamChunker, StreamSplitter, UploadResume } from './stream-chunker'
...
export function isUploadError(result: StorageResult<any>): result is { success: false; error: StorageError } {
  return !result.success && !!result.error
}

export function isUploadSuccess<T>(result: StorageResult<T>): result is { success: true; data: T } {
...
export function getOptimalChunkSize(
  ...
  const defaultSize = providerDefaults[provider as keyof typeof providerDefaults] || CHUNK_SIZES.GOOGLE_DRIVE
  
  // For small files, use smaller chunks
  if (fileSize < 10 * 1024 * 1024) { // 10MB
    return Math.min(defaultSize, Math.max(256 * 1024, Math.ceil(fileSize / 10)))
  }
  
  // For slow connections, use smaller chunks
  if (connectionSpeed && connectionSpeed < 1024 * 1024) { // 1 Mbps
...
export function validateProviderConfig(
  ...
  if (provider === 'google_drive') {
    ...
    const hasRequired = requiredScopes.every(scope => config.scopes.includes(scope))
    if (!hasRequired) {
      errors.push(`Missing required scopes for Google Drive: ${requiredScopes.join(', ')}`)
    }
  }
...
export function createMemoryEfficientTransform(): TransformStream<Uint8Array, Uint8Array> {
  let bufferSize = 0
